...
The docker datasource is neither: its central rule only adds labels and never touches `automerge`, because every manager it spans (dockerfile, kubernetes, helm, kustomize, …) already carries its own catch-all under row 2. A docker-datasource dependency's automerge decision is entirely the matching manager's, so the datasource rule doesn't need one.

What no central rule may do any more is **automerge a hand-picked list of package names** — not even `docker/dockerfile`, which was the one exception kept when the original allowlists moved out; it has since moved out too.
...
- **They reuse the central group's slug wherever that group automerges** — `python-minor`, `ansible-galaxy-minor`, `gitlab-ci-minor` and `otel-builder-minor`. An opt-in then shares a branch with the central group rather than opening a second MR, and one slug is one name, so the `groupName` must match the central one exactly. Where the central group is a catch-all that says `automerge: false` — helm, kustomize, argocd, terraform — the opt-in carries its own `*-automerge` slug and is the only rule that uses it. That split is not cosmetic: renovate sets a grouped branch's `automerge` to `upgrades.every((upgrade) => upgrade.automerge)` (`dist/workers/repository/updates/generate.js:247`), so an opt-in that joined a catch-all's branch would stop automerging the moment any other dependency landed on it. `test/presets.test.ts` (`never shares a group slug between an automerging and a non-automerging rule`) enforces it. `datasource-docker-automerge-*` falls in this second bucket too, even though its `docker-{minor,major}` slug predates the `*-automerge` naming convention: the central docker datasource rule never automerges anything, so there is no central group left for it to share a branch with.
...
The docker datasource is not in this table any more: its only central rule adds labels and never sets `automerge`. Automerging a docker-datasource dependency is entirely up to whichever manager's own policy matched it (row 2 for dockerfile, kubernetes, helm, kustomize, …), or the `datasource-docker-automerge-*` opt-in.
...
**No central rule automerges a package by name any more**, and `never automerges a central package-name allowlist` enforces that with no exceptions. The node build and docs groups also name packages, but only to route them into a `build:` or `docs:` merge request: every devDependency automerges either way, so they are exempt from that check.
