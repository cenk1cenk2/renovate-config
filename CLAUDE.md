...
- **`default-with-test-paths` replaces `default/default`; it is never extended beside it.** `config:recommended` ignores `**/test/**`, `**/tests/**` and `**/__tests__/**` through `:ignoreModulesAndTests`. The preset is `extends: [default]` plus `ignorePresets: [':ignoreModulesAndTests']`, and renovate applies `ignorePresets` only to the subtree below the preset that declares it (`resolveConfigPresets` in `dist/config/presets/index.js` passes the list down, never sideways). So beside `default` it filters nothing, because the ignore list is already resolved, and a sibling re-extending `config:recommended` would replay its `packageRules` after every central rule — both measured. Two more edges: a consumer that sets its own `ignorePresets` replaces this one (an inherited non-empty list wins), and `ignorePaths` falls back to renovate's default, which drops the `vendor`, `examples` and `__fixtures__` ignores along with the test ones. `test/resolution.test.ts` resolves the real graph through renovate to hold this; it mocks the `local>` source, which is why `vitest.config.ts` inlines renovate.
