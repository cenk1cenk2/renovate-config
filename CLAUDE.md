...
- **`default` manages test directories.** `config:recommended` ignores `**/test/**`, `**/tests/**` and `**/__tests__/**` through `:ignoreModulesAndTests`, so a `tests/go.mod` was never updated. `default.ts` carries `ignorePresets: [':ignoreModulesAndTests']`, which drops that preset for every repository extending `default/default`. Renovate applies `ignorePresets` only to the subtree below the preset that declares it (`resolveConfigPresets` in `dist/config/presets/index.js` reads it from the config being resolved, passes it down, then deletes it), so the filter must sit on a preset above `config:recommended` — a preset extended beside `default` filters nothing, measured. The trade-offs: `ignorePaths` falls back to renovate's default (`node_modules`, `bower_components`), so `vendor`, `examples` and `__fixtures__` are no longer ignored either; a repository that extends `base` directly keeps the upstream ignores; and a repository setting its own `ignorePresets` replaces this list, because an inherited non-empty list wins — it must restate `:ignoreModulesAndTests` to keep the behaviour. `test/resolution.test.ts` resolves the real graph through renovate to hold this; it mocks the `local>` source, which is why `vitest.config.ts` inlines renovate.
...
| hk                                     | `managers/hk/manager.ts`               | `hk-minor`, for the `custom.regex` `hk.pkl` schema rule                                |
...
- **An `hk.pkl` pins hk's schema twice per line** — `.../releases/download/v2.0.1/hk@2.0.1#/Config.pkl` — and a pkl package URI cannot float, so `managers/hk/manager.ts` carries one `matchString` per occurrence. Renovate rewrites a custom-manager dependency through the exact string it matched (`replaceString`), and `getUpdatedPackageFiles` walks the branch's upgrades over the content the previous one produced, so the duplicates resolve one at a time by `depIndex`. The two match strings must not overlap: a single string spanning both pins would hold the version twice and `doAutoReplace` only replaces the first. `test/hk.test.ts` replays that loop through renovate's own `doAutoReplace` to prove every pin moves. The version is captured bare on both sides and `extractVersionTemplate` strips the tag's `v`, so the literal `v` in the release path stays put.
- **`default.json` is prettier-ignored.** It is generated, and prettier's formatting disagrees with the generator's. Without the ignore entry the `hk` pre-commit hook's prettier step rewrites the file on every commit, so the committed file would no longer match the generator output.
