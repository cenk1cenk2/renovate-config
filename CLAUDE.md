...
**The policy in one line: a central rule may automerge a whole manager, never a hand-picked list of package names.** Whether a manager's non-breaking updates are safe unattended is the same call in every repository, so it stays here; whether *this chart* is safe unattended is a per-repository call, so it lives in the repository. The targeted allowlists that used to sit in the helm, kustomize, argocd and docker groups moved out to the parameterized presets for exactly that reason — `docker/dockerfile` stayed because it is generic, not because it was grandfathered.

Three shapes, and which one a manager has is the whole policy:

1. **Generic group, automerges** — one rule over the manager's whole minor/patch surface (`matchPackageNames: ['*']`, a dep-type split, or a ring), `automerge: true`. node, go, python, gitlab-ci, ansible-galaxy, otel-builder and the docker datasource work this way.
2. **Explicit catch-all, does not automerge** — helm, kustomize, argocd and terraform. `createMultiDirectoryGroupRule()` emits `automerge: false` on every rule it builds, so the group states the refusal rather than inheriting it, and the `automerge: true` rule lives in the parameterized preset the consuming repository extends, which renovate appends after the whole central config and which therefore wins on last-match.
3. **No automerge rule at all** — kubernetes, dockerfile and rust. Their manager presets carry an identity rule and nothing else, so `automerge` is never set centrally and renovate's own default (`false`) stands. Same outcome as row 2 without the explicit statement; their opt-in presets work identically. Do not add an `automerge: false` rule to make them look like row 2 — it would be a no-op, and a broad new rule is a labelling and grouping hazard for no gain.
...
`test/presets.test.ts` pins that list in `CENTRAL_AUTOMERGE` (`automerges centrally only where the inventory says so`), so a central automerge appearing or disappearing fails the suite until the list is updated deliberately.
