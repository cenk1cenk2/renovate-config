...
| ---------------------------------------- | ----------------------------- | ---------------------------------------------- | ------- | --------------------------------------- | ------------------------------------------------------------ |
| `manager-helm-automerge-*`               | `managers/helm/`              | `minor`, `patch`                               | M       | `helm-{minor,major}-automerge`          | its own slug, not the central group's                        |
...
| `manager-ansible-galaxy-automerge-*`     | `managers/ansible-galaxy/`    | `minor`, `patch`, `pin`, `digest`              | S       | `ansible-galaxy-minor-automerge`, `ansible-galaxy-major` | `matchDepTypes: ['collections', 'roles']`, `DAILY`; minor keeps the central group's `[skip ci]`, major does not |
| `manager-gitlab-ci-automerge-*`          | `managers/gitlab-ci/`         | `minor`, `patch`, `pin`, `digest`              | S       | `gitlab-ci-minor-automerge`, `gitlab-ci-major` | matches `gitlabci` and `gitlabci-include`, `ANY`      |
| `manager-gitlab-ci-custom-automerge-*`   | `managers/gitlab-ci/custom-`  | `minor`, `patch`, `pin`, `digest`              | S       | `gitlab-ci-minor-automerge`, `gitlab-ci-major` | `custom.regex` scoped by the gitlab-ci monorepo dep type |
| `manager-otel-builder-automerge-*`       | `managers/otel-builder/`      | `minor`, `patch`, `digest`                     | S       | `otel-builder-minor-automerge`, `otel-builder-major` | `DAILY`                                       |
| `datasource-docker-automerge-*`          | `datasources/docker/`         | `minor`, `patch`, `pin`, `digest`              | S       | `docker-{minor,major}`                  | `matchDatasources: ['docker']`, `ANY`; the only pair that reuses a central slug |
...
- **They carry their own `*-automerge` slug, never the central group's.** Renovate sets a grouped branch's `automerge` to `upgrades.every((upgrade) => upgrade.automerge)` (`dist/workers/repository/updates/generate.js:247`), and every central group now says `automerge: false`, so an opt-in that joined one would stop automerging the moment any other dependency landed on that branch. `docker-minor` is the sole exception: the only central rule left on it is the `docker/dockerfile` automerge, so both rules on that slug automerge. One slug is one name, so the docker opt-in's `groupName` must match the central one exactly. `test/presets.test.ts` (`never shares a group slug between an automerging and a non-automerging rule`) enforces the split.
- **`manager-node-automerge-*` and `manager-go-automerge-*` carry no `groupSlug` and no `schedule`.** Node groups by dep type (`dev`, `build`, `docs`, `peer`, `package-manager`) and both node and go group by ring. `groupSlug`, `groupName` and `schedule` are all last-match-wins, so an opt-in that named a group would pull the package out of the MR it belongs in and discard its ring schedule. Those two presets flip `automerge` and nothing else.
  ...
  **The consequence, since the central node and go groups stopped automerging:** an opted-in package stays on its dep-type or ring branch, and that branch automerges only when every package on it is opted in too (`generate.js:247` again). For a repository with a handful of node dependencies that is reachable; for a large one it is not, and the opt-in reads as a no-op. Giving these two their own slug would fix it and cost the dep-type and ring grouping — an open trade-off, not an oversight.
...
**One package name: `docker/dockerfile`**, in `datasources/docker/datasource.ts`. It is the Dockerfile syntax directive, not an application image — the frontend every repository already builds with, so a per-repository opt-in would be the same decision in every repository. `test/presets.test.ts` (`only automerges the dockerfile syntax directive centrally`) walks everything reachable from `default` and fails on any other `automerge: true`.

`lockFileMaintenance` in `lock-file.ts` is the standing exemption the test skips: it is its own update type and carries no version bump.

Everything else — including the node, go, gitlab-ci and ansible-galaxy groups, which used to automerge every minor update rather than an allowlist — now says `automerge: false` and waits for the consuming repository to extend the matching `*-automerge-*` preset. Every one of those managers has a working opt-in pair, so nothing lost its route to automerge; what changed is that the route is now declared per repository.
